...
int main(int argc, char **argv)
{
	...
	if (-1 == pcap_loop(hPcap, -1, pcap_handle, NULL)) { /* 开始捕获数据包 */
		printf("!! Failed to capture packets, please check your network connection!\n");
#ifndef NO_NOTIFY
		if (showNotify)
			show_notify("MentoHUST - Error", "Failed to capture packets, please check your network connection!");
#endif
	}
	exit(EXIT_FAILURE);
}
...
static void exit_handle(void)
{
	...
	free_libpcap();
#endif
	printf(">> The certification has quit.\n");
}
...
static void sig_handle(int sig)
{
	if (sig == SIGALRM)	 /* 定时器 */
	{
		if (-1 == switchState(state))
		{
			...
			printf("!! Send packet failed, please check your network connection！\n");
#ifndef NO_NOTIFY
			if (showNotify)
				show_notify("MentoHUST - Error", "Send packet failed, please check your network connection！");
#endif
			exit(EXIT_FAILURE);
		}
	}
...
static void pcap_handle(u_char *user, const struct pcap_pkthdr *h, const u_char *buf)
{
	...
	if (buf[0x0c]==0x88 && buf[0x0d]==0x8e) {
		...
		if (buf[0x0F]==0x00 && buf[0x12]==0x01 && buf[0x16]==0x01) {	/* 验证用户名 */
		...
		else if (buf[0x0F]==0x00 && buf[0x12]==0x03) {	/* Authentication is successful */
			...
			switchState(ID_ECHO);
		else if (buf[0x0F]==0x00 && buf[0x12]==0x04) {  /* Authentication failure或被踢下线 */
			if (state==ID_WAITECHO || state==ID_ECHO) {
				printf(">> Certification dropped, began to reconnect!\n");
				...
				if (maxFail && ++failCount>=maxFail) {
					printf(">> Continuous authentication failed %u exit certification。\n", maxFail);
					exit(EXIT_SUCCESS);
				}
				restart();
			}
...
#ifndef NO_NOTIFY
				if (showNotify) {
					sprintf(str, "Deceive source: %s", formatHex(buf+0x16, 6));
					show_notify("MentoHUST - ARP prompt", str);
				}
#endif
			}
		}
...
static void showRuijieMsg(const u_char *buf, unsigned bufLen)
{
	...
	if (length > 0)
	{
		...
		{
			printf("$$ System hint:\t%s\n", serverMsg);
#ifndef NO_NOTIFY
			if (showNotify)
				show_notify("MentoHUST - System hint", serverMsg);
#endif
			free(serverMsg);
		}
	}
	if ((length=0x1c+buf[0x1b]+0x69+39) < bufLen)
	{
		...
		if (length>0 && (serverMsg=gbk2utf(serverMsg, length))!=NULL)
		{
			printf("$$ Billing Tip:\t%s\n", serverMsg);
#ifndef NO_NOTIFY
			if (showNotify)
				show_notify("MentoHUST - Billing Tip", serverMsg);
#endif
			free(serverMsg);
		}
	}
