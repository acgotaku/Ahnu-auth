...
fileError:
	if (dataFile[strlen(dataFile)-1] != '/')
		printf("!! The selected file %s is invalid, the switch to the built-in data certification。\n", dataFile);
	return -1;
}
...
static int getAddress()
{
	...
	if (dhcpMode!=0 || mask==-1) {
		if (ioctl(sock, SIOCGIFNETMASK, &ifr) < 0)
			printf("!! ON NIC %sGET GateWay failure!\n", nic);
		else
...
getMACError:
	close(sock);
	printf("!! On NIC %s GET MACfailure!\n", nic);
	return -1;
}
...
static int Check(const u_char *md5Seed)	/* 客户端校验 */
{
	...
	if (value == -1) {
		printf("!! To lack 8021x.exe information, client-side validation can not continue！\n");
		return 1;
	}
	V2_check(md5Seed, final_str);
	printf("** V2 checksum value:\t%s\n", final_str);
...
pingError:
	perror("!! Ping host error, close the");
	if (sock != -1)
		close(sock);
	pingHost = 0;
	return 0;
