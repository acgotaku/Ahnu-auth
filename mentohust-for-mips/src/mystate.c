...
int switchState(int type)
{
	...
	if (sendCount>=MAX_SEND_COUNT && type!=ID_ECHO)  /* 超时太多次？ */
	{
		...
		{
		case ID_START:
			printf(">> Can not find server restart certification!\n");
			break;
		case ID_IDENTITY:
			printf(">> To send username timeout restart certification!\n");
			break;
		case ID_CHALLENGE:
			printf(">> Send password timeout restart certification!\n");
			break;
		case ID_WAITECHO:
			printf(">> Waiting for response packet timeout own response!\n");
...
static int renewIP()
{
	...
	printf(">> Getting IP...\n");
	system(dhcpScript);
	printf(">> The end of the operation.\n");
...
static int sendEchoPacket()
{
	...
	if (sendCount == 0)
	{
		...
		printf(">> Send a heartbeat packet to keep online...\n");
